-- SmolLM2-135M end-to-end demo (stage 2: weights + Luau BPE tokenizer).
-- Kernels are the v5 shipping set (x64_split4 matvec, precomputed RoPE, x16 qK dot).
-- All SmolLM2 dims are divisible by 64, so the kernels run unchanged.
--
-- Stage-2 features:
...
-- x64-unrolled matvec_q8. Straight-line 64-term FMA per iter.
-- Both lm_head (cols=192) and FFN down (cols=1024) are multiples of 64, so
-- the tail loop is inert. Kept as a safety net for future shapes.
...
function M.load_model(blob: buffer, runtime_max_ctx: number?)
	...
	local st = {
		...
		attn_out = buffer.create(H*F4), attn_h = buffer.create(H*F4),
		gate = buffer.create(I*F4), up = buffer.create(I*F4),
		ffn_out = buffer.create(H*F4),
...
function M.step(st, token_id: number): buffer
	...
	local eps, theta = cfg.rms_eps, cfg.rope_theta
	local inv_sqrt_hd = 1.0 / math.sqrt(HD)
	local pos = st.pos
	local x, h, q = st.x, st.h, st.q
	local k_tmp, v_tmp = st.k_tmp, st.v_tmp
	local attn_out, attn_h = st.attn_out, st.attn_h
	local gate, up, ffn_out = st.gate, st.up, st.ffn_out
	...
	for layer = 0, cfg.layers - 1 do
		...
		matvec_q8(Wk, Sk, h, k_tmp, KV, H)
		matvec_q8(Wv, Sv, h, v_tmp, KV, H)
		...
		matvec_q8(Wo, So, attn_out, attn_h, H, H)
		for i = 0, H - 1 do
			buffer.writef32(x, i * F4, buffer.readf32(x, i * F4) + buffer.readf32(attn_h, i * F4))
		end
		rmsnorm(x, Wfn, h, H, eps)
		matvec_q8(Wg, Sg, h, gate, I, H)
		matvec_q8(Wu, Su, h, up,   I, H)
		for i = 0, I - 1 do
			local g = buffer.readf32(gate, i * F4)
			local silu = g / (1.0 + math.exp(-g))
			buffer.writef32(gate, i * F4, silu * buffer.readf32(up, i * F4))
		end
		matvec_q8(Wd, Sd, gate, ffn_out, H, I)
		for i = 0, H - 1 do
			buffer.writef32(x, i * F4, buffer.readf32(x, i * F4) + buffer.readf32(ffn_out, i * F4))
		end
	end
...
-- Profile-instrumented step: same math as M.step with (a) debug.profilebegin/end
-- markers for Studio's microprofiler and (b) os.clock() accumulators summed into
-- M.phase_ms so totals can be printed after a run.
